...
INSERT INTO dental_clinic2 VALUES 
...
SELECT * FROM school2;
SELECT * FROM dental_clinic2;

# added first fulltext index
CREATE FULLTEXT INDEX ind_full_name
ON dental_clinic2 (name,location);

# added second fulltext index
CREATE FULLTEXT INDEX ind_full_numbers
ON dental_clinic2 (office_number, other_number);

# added first fulltext index
CREATE FULLTEXT INDEX ind_full_name
ON school2 (name,location);

# added second fulltext index
CREATE FULLTEXT INDEX ind_full_numbers
ON school2 (office_number,other_number);

SELECT * FROM school2 WHERE MATCH `name` AGAINST ('high' IN BOOLEAN MODE);

SELECT * FROM dental_clinic2 WHERE MATCH (name, location) AGAINST ('1400' IN BOOLEAN MODE);
